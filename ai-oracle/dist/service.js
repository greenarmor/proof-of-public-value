...
const CREDS_PATH = (0, path_1.join)(__dirname, "..", ".dev-logs", "newrolecreden.md");
// LLM config
...
function isMaybeIpfsHash(s) {
    return !!s && (s.startsWith("Qm") || s.startsWith("baf"));
}
async function fetchIpfsContent(hash) {
...
function detectIpfsFileType(buffer) {
    if (buffer.length < 4)
        return "unknown";
    // Image signatures
    ...
    try {
        ...
        if (printable / Math.min(buffer.length, 1024) > 0.7)
            return "text";
    }
    catch { /* fall through */ }
    return "binary";
}
function analyzeIpfsText(buffer) {
    const flags = [];
    try {
...
async function analyzeIpfsContent(dataHash, metadataStr) {
    const result = {
        hash: dataHash,
        type: "unknown",
        size: 0,
    ...
    if (!isMaybeIpfsHash(hash)) {
        ...
        if (match)
            ...
            return result; // No IPFS hash found
    }
    ...
    switch (result.type) {
        ...
        case "document": {
            if (content.length < 2000)
                result.flags.push("suspicious_small_document");
            break;
        }
        case "text": {
...
function getSecretKey() {
    if (process.env.AI_AUDITOR_SECRET)
        return process.env.AI_AUDITOR_SECRET;
    try {
...
const AI_AUDITOR_PUBLIC = stellar_sdk_1.Keypair.fromSecret(AI_AUDITOR_SECRET).publicKey();
...
function shouldSubmit(key, data) {
...
async function sdkInvoke(contractId, method, scvArgs) {
    if (!AI_AUDITOR_SECRET) {
        console.error("  AI_AUDITOR_SECRET not set");
        return false;
    }
    try {
        const { Keypair, Contract, TransactionBuilder, rpc } = await Promise.resolve().then(() => __importStar(require("@stellar/stellar-sdk")));
...
async function sdkSubmitOracle(method, scvArgs) {
...
async function sdkVerifyCommunityReport(reportId, weight = 30) {
    const { Address, nativeToScVal } = await Promise.resolve().then(() => __importStar(require("@stellar/stellar-sdk")));
...
function cli(cmd) {
    try {
        const raw = (0, child_process_1.execSync)(`${STELLAR} ${cmd}`, { ...opts, stdio: ["pipe", "pipe", "pipe"] }).toString().trim();
        ...
        if (stderr)
            console.error(`  CLI stderr: ${stderr.slice(0, 150)}`);
        return "";
    }
}
// ── LLM Fraud Analysis ──────────────────────────────────
async function analyzeWithLLM(pvoTitle, milestoneTitle, milestoneDesc, budget, evidence) {
...
// ── Rule-Based Fallback ─────────────────────────────────
function analyzeRuleBased(evidence) {
    const flags = [];
    let riskScore = 0;
...
function mapFlagsToIndicators(flags) {
    const indicators = new Set();
    for (const f of flags) {
        const mapped = FRAUD_INDICATOR_MAP[f];
        if (mapped)
            indicators.add(mapped);
    }
    return [...indicators];
}
...
function getGeoRisk(municipality) {
...
const rewardedReports = new Set();
// Persist rewarded reports to disk to prevent re-reward on restart
const REWARDS_FILE = `${HOME}/popv-rewarded-reports.json`;
try {
    const saved = (0, fs_1.readFileSync)(REWARDS_FILE, 'utf-8');
    JSON.parse(saved).forEach((k) => rewardedReports.add(k));
}
catch { }
function persistRewards() {
    try {
        (0, fs_1.writeFileSync)(REWARDS_FILE, JSON.stringify([...rewardedReports]));
    }
    catch { }
}
function getRewardTier(confidenceRating) {
    if (confidenceRating >= 96)
        return { tier: "Guardian", pct: 0.00001 };
    if (confidenceRating >= 81)
        return { tier: "Elite", pct: 0.000007 };
    if (confidenceRating >= 51)
        return { tier: "Trusted", pct: 0.000005 };
    if (confidenceRating >= 21)
        return { tier: "Rising", pct: 0.000003 };
...
try {
    const saved = (0, fs_1.readFileSync)(PENDING_FILE, "utf-8");
    JSON.parse(saved).forEach((r) => pendingReviews.set(r.escrowId, r));
}
catch { }
function persistPendingReviews() {
    try {
        (0, fs_1.writeFileSync)(PENDING_FILE, JSON.stringify([...pendingReviews.values()], null, 2));
    }
    catch { }
}
function logPendingReview(escrowId, pvoId) {
    const review = {
        escrowId,
        pvoId,
        pvoTitle: "",
...
function resolvePendingReview(escrowId) {
    pendingReviews.delete(escrowId);
    persistPendingReviews();
}
async function rewardCitizenForReport(citizenAddress, reportId, pvoId, winningBidStroops) {
    const rewardKey = `${reportId}:${citizenAddress}`;
    if (rewardedReports.has(rewardKey)) {
        console.log(`  [Reward] Report #${reportId} already in dedup set, skipping`);
        return false;
    }
    if (!CENTRAL_BANK_SECRET) {
        console.log("  [Reward] CENTRAL_BANK_SECRET not set, skipping");
        return false;
    }
    try {
        ...
        try {
            ...
            console.log(`  [Reward] queryContract community_oracle failed: ${e.message?.slice(0, 120)}`);
        }
        ...
        if (confidence <= 50) {
            try {
                ...
                if (repConfidence > confidence)
                    confidence = repConfidence;
            }
            catch (e) {
                console.log(`  [Reward] queryContract reputation fallback failed: ${e.message?.slice(0, 120)}`);
            }
        }
        ...
        if (!account) {
            console.log("  [Reward] CentralBank account not reachable, skipping");
            return false;
        }
...
function submitEscrowGate5(escrowId, passed) {
...
function submitEscrowGate3(escrowId) {
    const { Address, xdr } = require("@stellar/stellar-sdk");
    ...
    sdkInvoke(CONTRACT_IDS.escrow, "community_oracle_validate", [
        ...
        if (ok) {
            console.log(`  [Gate 3] Submitted on-chain`);
        }
        else {
            console.error(`  [Gate 3] Failed (may need more verified reports or escrow not funded)`);
        }
    });
}
// ── On-Chain: AI Oracle Submissions ─────────────────────
function submitFraudDetection(pvoId, riskScore, indicators, confidence, evidenceHash) {
    const { Address, xdr } = require("@stellar/stellar-sdk");
...
function submitRiskPrediction(contractor, delayProb, overrunProb, riskCategory, confidence) {
    const { Address, xdr } = require("@stellar/stellar-sdk");
...
function submitImageVerification(evidenceId, progressPercent, authenticityScore, summary) {
    const { Address, xdr } = require("@stellar/stellar-sdk");
...
function submitDigitalTwin(pvoId, expectedCost, materialIdx, laborIdx, deviation) {
    const { Address, xdr } = require("@stellar/stellar-sdk");
...
function submitGeoRisk(pvoId, region, flood, seismic, landslide) {
    const { Address, xdr } = require("@stellar/stellar-sdk");
...
function submitGpsValidation(evidenceId, expectedLat, expectedLon, reportedLat, reportedLon, maxDistM) {
...
function getFraudCount() {
    const raw = cli(`contract invoke --id ${CONTRACT_IDS.ai_oracle} --source ${AI_SOURCE_KEY} --network testnet -- get_fraud_count`);
    const m = raw.match(/(\d+)/);
    return m ? parseInt(m[1]) : 0;
}
function getFraudByPvo(pvoId) {
    const raw = cli(`contract invoke --id ${CONTRACT_IDS.ai_oracle} --source ${AI_SOURCE_KEY} --network testnet -- get_fraud_by_pvo --pvo_id ${pvoId}`);
    if (!raw)
        return [];
    try {
...
function hasGeoRisk(pvoId) {
    const raw = cli(`contract invoke --id ${CONTRACT_IDS.ai_oracle} --source ${AI_SOURCE_KEY} --network testnet -- get_geo_risk --pvo_id ${pvoId}`);
    if (!raw)
        return false;
    try {
        const parsed = JSON.parse(raw);
...
function hasDigitalTwin(pvoId) {
    const raw = cli(`contract invoke --id ${CONTRACT_IDS.ai_oracle} --source ${AI_SOURCE_KEY} --network testnet -- get_digital_twin --pvo_id ${pvoId}`);
    if (!raw)
        return false;
    try {
        const parsed = JSON.parse(raw);
...
function extractGpsFromDesc(description) {
...
// ── Escrow Query ────────────────────────────────────────
function getEscrowsByPvo(pvoId) {
    const raw = cli(`contract invoke --id ${CONTRACT_IDS.escrow} --source ${AI_SOURCE_KEY} --network testnet -- get_escrows_by_pvo --pvo_id ${pvoId}`);
    if (!raw)
        return [];
    try {
        const parsed = JSON.parse(raw);
...
function queryContract(contractKey, method, args = "") {
    const raw = cli(`contract invoke --id ${CONTRACT_IDS[contractKey]} --source ${AI_SOURCE_KEY} --network testnet -- ${method} ${args}`);
    if (!raw)
        return null;
    try {
        const parsed = JSON.parse(raw);
...
async function collectForensicData(pvoId, pvo, milestones) {
    const flags = [];
    const timeline = [];
    ...
    for (let t = 1; t <= Number(tenderCount); t++) {
        ...
        if (tender && Number(tender.pvo_id) === pvoId) {
            ...
            const prices = bids.map((b) => Number(b.price || 0));
            if (prices.length >= 2) {
    ...
    const awardedTenders = tenders.filter((t) => {
        const st = typeof t.status === "string" ? t.status : t.status?.tag ?? "";
        return st === "Awarded" || !!t.winner;
    });
    const totalEvidenceAll = milestones.reduce((s, m) => s + (m.submitted_evidence || []).length, 0);
    ...
    for (const cr of communityReports) {
        ...
        if (cr.verified && cr.citizen) {
            ...
            if (rewardBudget > 0) {
                try {
                    const rewarded = await rewardCitizenForReport(String(cr.citizen), Number(cr.id || 0), pvoId, rewardBudget);
                    if (!rewarded) {
    ...
    for (const e of escrows) {
        ...
        if (engineerApproved) {
            ...
            const reportsForMs = communityReports.filter((cr) => Number(cr.milestone_id || 0) === escrowMilestone && cr.verified);
            ...
            const totalReportsForMs = communityReports.filter((cr) => Number(cr.milestone_id || 0) === escrowMilestone);
    ...
    let contractorReputation = null;
    let contractorComplaints = [];
    if (contractor && contractor.length > 10) {
        ...
        if (!contractorReputation) {
            ...
            if (Number(rep.reputation_score || 0) < 40)
                flags.push(`LowReputation:${rep.reputation_score}`);
            if (Number(rep.safety_violations || 0) > 0)
                flags.push(`SafetyViolations:${rep.safety_violations}`);
            if (Number(rep.audit_findings || 0) > 2)
                flags.push(`MultipleAuditFindings:${rep.audit_findings}`);
    ...
    let actualBudget = null;
    let actualBudgetPerMs = null;
    for (const tender of tenders) {
        ...
        if ((tStatus === "Awarded" || tender.winner) && bidsByTender[tender.id]) {
            ...
            if (bids.length > 0) {
                const winner = bids.reduce((best, b) => (Number(b.final_score || 0) > Number(best.final_score || 0)) ? b : best, bids[0]);
    ...
    if (escrows.length > 0 && milestones.length > 0) {
        for (const e of escrows) {
            const ms = milestones.find((m) => Number(m.id) === Number(e.milestone_id));
            if (!ms)
                continue;
    ...
    const hasFundedEscrows = escrows.some((e) => {
    ...
    const hasAnyEvidence = milestones.some((m) => (m.submitted_evidence || []).length > 0);
...
function forensicFlagsToIndicators(flags) {
    const indicators = new Set();
    for (const f of flags) {
        const lower = f.toLowerCase();
        if (lower.includes("ghost"))
            indicators.add("GhostProject");
        if (lower.includes("collusion") || lower.includes("clustering") || lower.includes("singlebid"))
            indicators.add("CollusionPattern");
        if (lower.includes("mismatch") || lower.includes("gap"))
            indicators.add("AbnormalBudgetGrowth");
        if (lower.includes("duplicate"))
            indicators.add("DuplicateInvoice");
        if (lower.includes("timing") || lower.includes("unusual"))
            indicators.add("UnusualPaymentTiming");
        if (lower.includes("shell"))
            indicators.add("ShellCompanyRisk");
        if (lower.includes("inflation") || lower.includes("material"))
            indicators.add("MaterialCostInflation");
        if (lower.includes("repeated") || lower.includes("contractor"))
            indicators.add("RepeatedContractorWin");
    }
    return [...indicators];
}
// ── Comprehensive Forensic PVO Analysis ──────────────────
async function analyzePvo(caseFile) {
    const { pvoId, pvo, milestones, escrows, grants, tenders, violations, communityReports, contractorReputation, contractorComplaints, valueScore, auditHistory, flags, timeline } = caseFile;
    ...
    if (flags.length > 0)
        console.log(`  [Forensic Flags] ${flags.join(", ")}`);
    ...
    if (!hasDigitalTwin(pvoId)) {
        const budgetForTwin = caseFile.actualBudget
            ?? milestones.reduce((s, m) => s + Number(m.budget || 0), 0);
    ...
    const existingFraud = getFraudByPvo(pvoId);
    {
        ...
        if (pendingRatio > 0.5)
            factors.push(`${pendingRatio > 0.75 ? "Majority" : "Half"} of milestones still pending`);
        // Factor: Evidence coverage
        const milestonesWithEvidence = milestones.filter((m) => (m.submitted_evidence ?? []).length > 0).length;
        ...
        if (communityReports.length === 0 && milestones.some((m) => {
            const st = typeof m.status === "string" ? m.status : m.status?.tag ?? "";
    ...
    for (const m of milestones) {
        ...
        for (const ev of submitted) {
            ...
            // 5. Submit Image Verification for each evidence item
            {
                ...
                if (isMaybeIpfsHash(dataHash) || isMaybeIpfsHash(metadataStr)) {
                    ...
                    if (ipfsResult.flags.includes("ipfs_unreachable"))
                        adjustedAuth -= 20;
                    if (ipfsResult.flags.includes("tiny_image_file"))
                        adjustedAuth -= 10;
                    if (ipfsResult.flags.includes("no_exif_metadata"))
                        adjustedAuth -= 8;
                    if (ipfsResult.flags.includes("suspicious_small_document"))
                        adjustedAuth -= 15;
                    if (ipfsResult.flags.includes("binary_format_unverifiable"))
                        adjustedAuth -= 5;
                    if (ipfsResult.flags.includes("suspicious_metadata"))
                        adjustedAuth -= 15;
                    if (ipfsResult.flags.includes("low_text_density"))
                        adjustedAuth -= 10;
                    ...
                    if (shouldSubmit(imgKey, imgData)) {
                        ...
                        console.log(`  [Image] Ev #${ev.id}: Unchanged, skipped`);
                    }
                }
                else {
        ...
        if (submitted.length > 0) {
            ...
            if (LLM_API_KEY) {
                try {
                    result = await analyzeWithLLM(pvoTitle, m.title ?? "", m.description ?? "", milestoneBudget, evidence);
            ...
            for (const f of flags) {
                const lower = f.toLowerCase();
                if (lower.includes("ghost"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 40);
                if (lower.includes("collusion") || lower.includes("clustering"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 30);
                if (lower.includes("criticalviolation"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 25);
                if (lower.includes("safety"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 20);
                if (lower.includes("fundinggap"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 15);
                if (lower.includes("noreputation"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 10);
                if (lower.includes("lowrep"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 10);
                if (lower.includes("singlebid"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 10);
                if (lower.includes("highdelay"))
                    forensicRiskAdjust = Math.max(forensicRiskAdjust, 10);
...
function detectCollusion(allPvoData) {
    const flags = [];
    const contractorPvos = {};
    for (const d of allPvoData) {
        if (!d.contractor || d.contractor.length < 5)
            continue;
        // Only count PVOs where this contractor actually won a tender (not placeholder)
        if (!d.wonTender)
            continue;
        if (!contractorPvos[d.contractor])
            contractorPvos[d.contractor] = [];
...
function checkEscrowGates() {
    const escCountRaw = cli(`contract invoke --id ${CONTRACT_IDS.escrow} --source ${AI_SOURCE_KEY} --network testnet -- get_escrow_count`);
    let escCount = parseInt(escCountRaw.match(/(\d+)/)?.[1] ?? "0");
    if (isNaN(escCount) || escCount === 0)
        escCount = parseInt(escCountRaw) || 0;
    if (escCount === 0)
        return;
    for (let escrowId = 1; escrowId <= escCount; escrowId++) {
        const raw = cli(`contract invoke --id ${CONTRACT_IDS.escrow} --source ${AI_SOURCE_KEY} --network testnet -- get_escrow --escrow_id ${escrowId}`);
        if (!raw)
            continue;
    ...
    while (escNones < 15) {
        const raw = cli(`contract invoke --id ${CONTRACT_IDS.escrow} --source ${AI_SOURCE_KEY} --network testnet -- get_escrow --escrow_id ${escId}`);
        if (!raw) {
            escNones++;
            escId++;
            continue;
...
function processEscrowGates(escrowId, raw) {
    try {
        const parsed = JSON.parse(raw);
        const escrow = parsed.result || parsed;
        if (!escrow)
        ...
        if (escrow.conditions?.ai_risk_check === true)
            return;
        ...
        if (commCount < commRequired)
            return;
        if (status !== "CommunityVerified" && status !== "Ready" && status !== "OracleValidated")
            return;
...
async function poll() {
    ...
    try {
        const pvoCountRaw = cli(`contract invoke --id ${CONTRACT_IDS.pvo_core} --source ${AI_SOURCE_KEY} --network testnet -- get_pvo_count`);
        let pvoCount = parseInt(pvoCountRaw.match(/(\d+)/)?.[1] ?? "0");
        if (isNaN(pvoCount) || pvoCount === 0)
            pvoCount = parseInt(pvoCountRaw) || 0;
        if (pvoCount === 0) {
        ...
        for (let pvoId = 1; pvoId <= pvoCount; pvoId++) {
            try {
                const pvoRaw = cli(`contract invoke --id ${CONTRACT_IDS.pvo_core} --source ${AI_SOURCE_KEY} --network testnet -- get_pvo --pvo_id ${pvoId}`);
                if (!pvoRaw) {
                ...
                const milestonesRaw = cli(`contract invoke --id ${CONTRACT_IDS.pvo_core} --source ${AI_SOURCE_KEY} --network testnet -- get_pvo_milestones --pvo_id ${pvoId}`);
                let milestones = [];
                if (milestonesRaw) {
        ...
        while (pvoNones < 15) {
            try {
                const pvoRaw = cli(`contract invoke --id ${CONTRACT_IDS.pvo_core} --source ${AI_SOURCE_KEY} --network testnet -- get_pvo --pvo_id ${pvoScanId}`);
                if (!pvoRaw) {
                    pvoNones++;
                    pvoScanId++;
                    continue;
                ...
                pvoNones = 0;
                const milestonesRaw = cli(`contract invoke --id ${CONTRACT_IDS.pvo_core} --source ${AI_SOURCE_KEY} --network testnet -- get_pvo_milestones --pvo_id ${pvoScanId}`);
                let milestones = [];
                if (milestonesRaw) {
                    const mParsed = JSON.parse(milestonesRaw);
                    milestones = Array.isArray(mParsed) ? mParsed : (mParsed.result || []);
