...
export function BlockchainLoader({ text = "Syncing with Stellar testnet..." }: { text?: string }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[40vh] space-y-5 sm:space-y-6 px-3">
      {/* Blockchain block animation */}
      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap justify-center">
        {blocks.map((block) => (
          <div key={block.i} className="flex items-center gap-1.5 sm:gap-2">
            <div
              className={`w-9 h-9 sm:w-11 sm:h-11 rounded-lg border-2 flex items-center justify-center text-xs sm:text-sm font-mono transition-all duration-500 ${
            ...
            {block.i < 5 && (
              <div
                className={`w-4 h-0.5 sm:w-6 sm:h-0.5 transition-colors duration-500 ${
      ...
      {/* Rotating fact */}
      <div className="bg-white/80 backdrop-blur border border-slate-200 rounded-xl px-4 py-3 sm:px-5 max-w-[92vw] sm:max-w-md text-center transition-all duration-500 ease-in-out">
        <p className="text-xs sm:text-sm text-slate-500 leading-relaxed">💡 {FACTS[fact % FACTS.length]}</p>
      </div>
    </div>
  );
}
