...
export function CreatePphpTrustline({ address }: { address: string }) {
  ...
  const pphpIssuer = "GBRDP6UQ625API2MGOMSV3Z3ZWJIABCDCKGOOCOCJNNZYNZ32XYBBBHO";
    ...
    try {
      ...
      const tx = new TransactionBuilder(acct, { fee: "100000", networkPassphrase: NETWORK_PASSPHRASE })
        .addOperation(Operation.changeTrust({
          asset: new Asset("pPHP", pphpIssuer),
        }))
        .setTimeout(30).build();
  ...
  return (
    <div className="mb-4 p-3 bg-purple-50 border border-purple-200 rounded-xl">
      ...
      <p className="text-sm text-purple-700 mb-2">
        <strong>🪙 pPHP Tokens:</strong> Add a trustline to receive and hold pPHP in your wallet. Issued by CentralBank.
      </p>
      <button onClick={setup} disabled={loading}
        className="btn-primary text-xs px-4 py-2">
        {loading ? "Opening Freighter..." : "🔓 Add pPHP Trustline"}
      </button>
    </div>
  );
}
